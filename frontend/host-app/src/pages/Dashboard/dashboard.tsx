      ...
      >
        <div className="relative z-10 flex items-center justify-between h-full">
          <div className="flex flex-col justify-center">
            ...
            <p className="text-[var(--sidebar-text)] text-sm font-semibold uppercase tracking-wide">{stat.title}</p>
          </div>
          <div className="flex items-center justify-center opacity-80">
            <div className="text-5xl text-[var(--sidebar-text)]">{stat.icon}</div>
          </div>
        </div>
        <div className="absolute top-0 right-0 w-20 h-20 bg-[var(--primary-700)]/10 rounded-full -mr-10 -mt-10 z-0"></div>
        <div className="absolute bottom-0 left-0 w-16 h-16 bg-black/10 rounded-full -ml-8 -mb-8 z-0"></div>
      </div>
    ))}
  </div>
);
...
const SalesOverview = ({ salesData }: { salesData: { time: string; value: number }[] }) => {
  return (
    <div className="bg-[var(--background-secondary)] rounded-lg shadow-md p-6 border border-[var(--border-color)] hover:shadow-lg transition-shadow duration-300">
          ...
          <AreaChart data={salesData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
            <defs>
              ...
              <linearGradient id="colorValueSecondary" x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor="var(--surface-color)" stopOpacity={0.8} />
                <stop offset="95%" stopColor="var(--surface-color)" stopOpacity={0.1} />
              </linearGradient>
            </defs>
            ...
            <Area
              ...
              fill="url(#colorValue)"
              fillOpacity={1}
              dot={{ r: 4, fill: 'var(--primary-color)', strokeWidth: 2, stroke: 'var(--background-secondary)' }}
              activeDot={{ r: 6, fill: 'var(--primary-color)', strokeWidth: 2, stroke: 'var(--background-secondary)' }}
            />
          </AreaChart>
        </ResponsiveContainer>
      </div>
...
const TopSellingItems = ({ items }: { items: { name: string; orders: number; image: string }[] }) => (
  <div className="bg-[var(--background-secondary)] rounded-lg shadow-md p-4 border border-[var(--border-color)] hover:shadow-lg transition-shadow duration-300">
    <div className="flex justify-between items-center mb-3">
      ...
      <button className="text-[var(--primary-color)] hover:text-[var(--primary-600)] text-xs font-medium transition-colors duration-200">
        View All
      </button>
    </div>
    <div className="space-y-2">
      {items.slice(0, 3).map((item, index) => (
        <div key={index} className="flex items-center p-2 hover:bg-[var(--surface-color)] rounded-md transition-colors duration-200">
          ...
          <div className="ml-3 flex-1">
            <h4 className="font-semibold text-[var(--text-color)] text-sm">{item.name}</h4>
            <p className="text-[var(--text-secondary)] text-xs">{item.orders} orders</p>
          </div>
        </div>
      ))}
    </div>
  ...
  const stats = [
    {
      ...
      color: 'text-[var(--sidebar-text)]', // Use theme variable
      bgColor: 'bg-[var(--primary-color)]', // Use theme variable
      gradient: 'bg-gradient-to-br from-[var(--primary-color)] to-[var(--primary-600)]'
    },
    {
      title: 'Orders Done',
      value: ordersProcessed.toString(),
      icon: <FontAwesomeIcon icon={faClipboardList} />,
      color: 'text-[var(--sidebar-text)]',
      bgColor: 'bg-[var(--success-color)]',
      gradient: 'bg-gradient-to-br from-[var(--success-color)] to-[var(--success-color)]/80'
    },
    {
      title: 'Dine-In',
      ...
      color: 'text-[var(--sidebar-text)]',
      bgColor: 'bg-[var(--warning-color)]',
      gradient: 'bg-gradient-to-br from-[var(--warning-color)] to-[var(--warning-color)]/80'
    },
    {
      title: 'Takeaway',
      ...
      color: 'text-[var(--sidebar-text)]',
      bgColor: 'bg-[var(--error-color)]',
      gradient: 'bg-gradient-to-br from-[var(--error-color)] to-[var(--error-color)]/80'
    },
  ];
            ...
            <div className="text-right">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <input
                  ...
                  className="px-3 py-1 bg-[var(--background-secondary)] border border-[var(--border-color)] rounded-md text-xs text-[var(--text-color)] focus:outline-none focus:ring-2 focus:ring-[var(--primary-color)] transition-colors duration-200"
                />
                <input
                  type="date"
