    ...
    <div className="p-4">
      ...
      {isLoading.fetch ? (
        ...
        <div className="text-center py-8">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">No roles found</p>
        </div>
      ) : (
        <div className="space-y-2">
          {roles.map((role) => (
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-1">{role.name}</h4>
                  {role.description && (
                    <p className="text-xs text-gray-600 dark:text-gray-400 mb-1">{role.description}</p>
                  )}
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Permissions: {role.permissions?.length ? role.permissions.map(p => p.key).join(', ') : 'None'}
                  </p>
                </div>
