...
import {
    ...
    ArrowLeftIcon,
    ArrowRightIcon,
    MagnifyingGlassIcon, TagIcon, ShoppingBagIcon
...
export default function ProductList({
  ...
  return (
      <div>
          ...
          <div className="mb-4">
              <label className="block text-sm font-medium mb-1" style={{color: 'var(--text-secondary)'}}>Filter by
                  Category</label>
          ...
          {currentProducts.length > 0 ? (
              <div
                  className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3 w-full">
                  {currentProducts.map((product) => (
                      ...
                      >
                          <div className="relative h-1/2 cursor-pointer"
                               onClick={() => !isCategoryFormActive && onViewDetails(product)}>
                              {product.pictureUrl ? (
                                  <img
                          ...
                          <div className="p-2 flex flex-col justify-between h-1/2">
                              <div>
                                  <h3 className="text-md font-semibold truncate"
                                      style={{color: 'var(--text-color)'}}>{product.name}</h3>
                                  <div className="flex items-center justify-between mt-1">
                                      <p className="font-medium text-sm"
                                         style={{color: 'var(--primary-color)'}}>{product.displayPrice}</p>
                                      <p className="text-xs"
                                         style={{color: 'var(--text-secondary)'}}>{product.categoryName}</p>
                                  </div>
                              </div>
                              <div className="flex justify-between items-center mt-2">
                                  <label className="flex items-center space-x-2 cursor-pointer">
                                      <div className="relative">
                                          ...
                                          />
                                          <div
                                              className={`w-10 h-5 rounded-full transition duration-200 ${product.isActive ? 'bg-[var(--primary-color)]' : 'bg-[var(--background-secondary)]'}`}></div>
                                          <div
                                              className={`absolute top-0.5 left-0.5 w-4 h-4 bg-[var(--surface-color)] rounded-full shadow transition duration-200 transform ${product.isActive ? 'translate-x-5' : 'translate-x-0'}`}></div>
                                      </div>
                                      <span className="text-sm"
                                            style={{color: 'var(--text-secondary)'}}>{product.isActive ? 'Active' : 'Deactive'}</span>
                                  </label>
